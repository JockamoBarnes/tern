...
const navLinks = [
  ...
  { label: 'Contact',   href: 'mailto:hello@tern.app' },
];

const socialLinks = [
  {
    label: 'Instagram',
    href: 'https://instagram.com/tern.app',
    icon: (
  ...
  {
    label: 'LinkedIn',
    href: 'https://linkedin.com/company/tern-app',
    icon: (
...
export default function Footer() {
  return (
    <footer style={{ backgroundColor: 'var(--teal)', borderTop: '1px solid rgba(21,234,173,0.08)' }}>
      <div
        className="mx-auto px-6 md:px-12 pt-12 pb-8"
        style={{ maxWidth: 1200 }}
      >
        {/* Top row — wordmark + nav + social */}
        <div
          style={{
            display: 'flex',
            flexWrap: 'wrap',
            ...
            marginBottom: 32,
            paddingBottom: 28,
            borderBottom: '1px solid rgba(21,234,173,0.08)',
          }}
        >
          {/* Wordmark */}
          <Link href="/" aria-label="Tern home">
            <svg
              viewBox="0 0 892.5 279.5"
              style={{ height: 24, width: 'auto', display: 'block', opacity: 0.5 }}
              aria-hidden="true"
            >
              <defs>
          ...
          {/* Nav links */}
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '0 8px' }}>
            {navLinks.map((link, i) => (
              <span key={link.href} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {i < navLinks.length - 1 && (
                  <span style={{ color: 'rgba(238,247,248,0.2)', fontSize: 12 }}>·</span>
                )}
              </span>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {socialLinks.map(({ label, href, icon }) => (
              <a
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.color = '#15EAAD'; (e.currentTarget as HTMLAnchorElement).style.filter = 'drop-shadow(0 0 6px rgba(21,234,173,0.5))'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.color = 'rgba(238,247,248,0.4)'; (e.currentTarget as HTMLAnchorElement).style.filter = 'none'; }}
              >
                {icon}
              </a>
            ))}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 16px', alignItems: 'center' }}>
          <span style={{ fontFamily: 'var(--font-manrope)', fontWeight: 400, fontSize: 12, color: 'rgba(238,247,248,0.3)' }}>
            © {new Date().getFullYear()} Tern App Technologies Co. LLC
          </span>
          <span style={{ color: 'rgba(238,247,248,0.15)', fontSize: 12 }}>·</span>
          <Link href="/legal/privacy" style={{ fontFamily: 'var(--font-manrope)', fontWeight: 400, fontSize: 12, color: 'rgba(238,247,248,0.3)', textDecoration: 'none' }}>
            Privacy Policy
          </Link>
          <span style={{ color: 'rgba(238,247,248,0.15)', fontSize: 12 }}>·</span>
          <Link href="/legal/terms" style={{ fontFamily: 'var(--font-manrope)', fontWeight: 400, fontSize: 12, color: 'rgba(238,247,248,0.3)', textDecoration: 'none' }}>
            Terms &amp; Conditions
          </Link>
        </div>
      </div>
